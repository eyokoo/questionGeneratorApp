...
let questionById = (req, res) => {
  ...
  connection.query(sql, params, (error, rows) => {
    ...
    if (!rows || rows.length == 0) {
      ...
      res.sendStatus(404).send(`Question no found: Question Id: ${id}`);
      return;
    }
    res.json(rows[0]);
  });
...
let questionByC = (req, res) => {
  ...
  let sql =
    "SELECT Q.id, question, C.category FROM questions_table AS Q INNER JOIN categories_table AS C ON C.id = Q.category_id WHERE C.id = ?";
